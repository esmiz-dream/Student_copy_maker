 <?php
  session_start();
  if (!isset($_SESSION["roll"])) {
    header("location: error.php");
    exit; // prevent further execution, should there be more code that follows
  }

  //include 'grade_calculation.php';
  include 'db_connection.php';
  $conn = OpenCon();
  $student = (int)$_POST['student_id'];

  $stud_info_sql = "select * from student where id=$student";
  $student_info = mysqli_query($conn, $stud_info_sql);
  $student_info_row = mysqli_fetch_all($student_info, MYSQLI_ASSOC);

  $stud_grade_sql = "select * from grade where student_id=$student";
  $student_grades = mysqli_query($conn, $stud_grade_sql);

  function get_status($cgpa)
  {

    if ($cgpa >= 3.75) {
      return  "Very great distinction.";
    } else if ($cgpa >= 3.5) {
      return  "Great distinction.";
    } else if ($cgpa >= 3.25) {
      return  "Distinction.";
    } else if ($cgpa >= 2.75) {
      return  "Passed";
    } else if ($cgpa >= 2.5) {
      return  "Warning";
    } else if ($cgpa >= 2.0) {
      return  "Academic dismissal for readmission ";
    } else if ($cgpa < 2.0) {
      return  "Academic dismissal";
    }
  }

  function get_crh($course)
  {
    $crh = 0;
    if ($course == 'ANAT201') {
      $crh = 4;
    } else if ($course == 'SOSA201') {
      $crh =   2;
    } else if ($course == 'FLEn101') {
      $crh =   3;
    } else if ($course == 'PHYL201') {
      $crh =   4;
    } else if ($course == 'Epsy201') {
      $crh =   3;
    } else if ($course == 'Chem202') {
      $crh =   3;
    }

    // year 1     semister II
    else if ($course == 'Micro222') {
      $crh = 4;
    } else if ($course == 'NURS282') {
      $crh =  2;
    } else if ($course == 'PARA321') {
      $crh =  2;
    } else if ($course == 'NURS231') {
      $crh = 3;
    } else if ($course == 'NURS202') {
      $crh =  4;
    } else if ($course == 'NURS211') {
      $crh =  2;
    }

    // year 2     semister I

    else if ($course == 'GeEd') {
      $crh = 4;
    } else if ($course == 'NURS331') {
      $crh =  2;
    } else if ($course == 'Pharm302') {
      $crh = 4;
    } else if ($course == 'NURS252') {
      $crh =  2;
    }

    // year 2     semister II

    else if ($course == 'NURS301') {
      $crh = 4;
    } else if ($course == 'NURS341') {
      $crh = 3;
    } else if ($course == 'MIDW321') {
      $crh = 4;
    } else if ($course == 'MIDW312') {
      $crh = 3;
    } else if ($course == 'Nurs311') {
      $crh = 3;
    } else if ($course == 'MIDW322') {
      $crh =   2;
    } else if ($course == 'MIDW471') {
      $crh =  2;
    }


    //year 3     semister I

    else if ($course == 'MIDW571') {
      $crh = 3;
    } else if ($course == 'MIDW451') {
      $crh = 3;
    } else if ($course == 'MIDW323') {
      $crh = 4;
    } else if ($course == 'NURS441') {
      $crh =  2;
    } else if ($course == 'MIDW412') {
      $crh =  4;
    } else if ($course == 'Comp2022') {
      $crh = 2;
    }


    //year 3     semister II

    else if ($course == 'MIDW372') {
      $crh =    2;
    } else if ($course == 'FLEN201') {
      $crh = 3;
    } else if ($course == 'NURS442') {
      $crh = 4;
    } else if ($course == 'NURS481') {
      $crh = 2;
    } else if ($course == 'MIDW472') {
      $crh = 3;
    } else if ($course == 'NURS521') {
      $crh =  2;
    }


    //year 4     semister I

    else if ($course == 'MIDW552') {
      $crh = 4;
    } else if ($course == 'MIDW431') {
      $crh =  3;
    } else if ($course == 'MIDW311') {
      $crh =  3;
    } else if ($course == 'STAT322') {
      $crh =    3;
    } else if ($course == 'MIDW482') {
      $crh = 2;
    } else if ($course == 'NURS462') {
      $crh =  2;
    }


    //year 4     semister II

    else if ($course == 'MIDW525') {
      $crh = 3;
    } else if ($course == 'MIDW552') {
      $crh =   2;
    } else if ($course == 'MIDW522') {
      $crh = 3;
    } else if ($course == 'PHYL202 ') {
      $crh = 2;
    } else if ($course == 'ANAT202') {
      $crh =    2;
    }


    //year 5     semister I

    else if ($course == 'MIDW542') {
      $crh = 2;
    } else if ($course == 'MIDW382') {
      $crh =  2;
    } else if ($course == 'NURS3144') {
      $crh =   2;
    } else if ($course == 'NURS561') {
      $crh =  2;
    } else if ($course == 'MIDW573') {
      $crh =  1;
    } else if ($course == 'MIDW532') {
      $crh =  2;
    }
    //year 5     semister II

    else if ($course == 'NURS522') {
      $crh =  2;
    } else if ($course == 'MIDW562') {
      $crh = 2;
    }
    return $crh;
  }

  function get_semester_gpa()
  {
  }

  function get_semester_status()
  {
  }

  function get_grade_point($grade, $course)
  {
    $grade = strtoupper($grade);
    $crh = get_crh($course);
    switch ($grade) {

      case "A+":
        return 4 * $crh;
        break;
      case "A":
        return 4 * $crh;
        break;
      case "A-":
        return 3.75 * $crh;
        break;
      case "B+":
        return 3.5 * $crh;
        break;
      case "B":
        return 3 * $crh;
        break;
      case "B-":
        return 2.75 * $crh;
        break;
      case "C+":
        return 2.5 * $crh;
        break;
      case "C":
        return 2 * $crh;
        break;
      case "C-":
        return 1.75 * $crh;
        break;
      case "D":
        return 1 * $crh;
        break;
      case "F":
        return 0 * $crh;
        break;
      default:
        "Error: plc inform to IT";
    }
  }

  if (!$student_grades) {
    die('Could not get data: ');
  }
  $grade_row = mysqli_fetch_all($student_grades, MYSQLI_ASSOC);

  ?>


 <!DOCTYPE html>
 <!-- saved from url=(0055)https://student.aambc.edu.et:8181/SMIS/TranscriptDegree -->
 <html>

 <head>
   <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
   <link rel="shortcut icon" href="https://student.aambc.edu.et:8181/SMIS/logo.jpg">
   <title>Students transcript|SMIS-Addis Ababa Medical and Business College</title>
   <style>
     a:link {
       text-decoration: none;
       color: blue;
       font-family: serif;
       font-size: 19px;
     }

     a:hover {

       color: red;
       background-color: white;
       font-family: serif;
       font-size: 19px;
     }

     a:visited {
       text-decoration: none;
       color: blue;
       font-family: serif;
       font-size: 19px;
     }
   </style>
   <style>
     .btn {
       display: inline-block;
       *display: inline;
       *zoom: 1;
       padding: 4px 10px 4px;
       margin-bottom: 0;
       font-size: 13px;
       color: #333333;
       text-align: center;
       text-shadow: 0 1px 1px rgba(255, 255, 255, 0.75);
       background-color: #f5f5f5;
       background-image: -moz-linear-gradient(top, #ffffff, #e6e6e6);
       background-image: -ms-linear-gradient(top, #ffffff, #e6e6e6);
       background-image: -webkit-gradient(linear, 0 0, 0 100%, from(#ffffff), to(#e6e6e6));
       background-image: -webkit-linear-gradient(top, #ffffff, #e6e6e6);
       background-image: -o-linear-gradient(top, #ffffff, #e6e6e6);
       background-image: linear-gradient(top, #ffffff, #e6e6e6);
       background-repeat: repeat-x;
       filter: progid:dximagetransform.microsoft.gradient(startColorstr=#ffffff, endColorstr=#e6e6e6, GradientType=0);
       border-color: #e6e6e6 #e6e6e6 #e6e6e6;
       border-color: rgba(0, 0, 0, 0.1) rgba(0, 0, 0, 0.1) rgba(0, 0, 0, 0.25);
       border: 1px solid #e6e6e6;
       -webkit-border-radius: 4px;
       -moz-border-radius: 4px;
       border-radius: 4px;
       -webkit-box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2), 0 1px 2px rgba(0, 0, 0, 0.05);
       -moz-box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2), 0 1px 2px rgba(0, 0, 0, 0.05);
       box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2), 0 1px 2px rgba(0, 0, 0, 0.05);
       cursor: pointer;
       *margin-left: .3em;
     }

     .btn:hover,
     .btn:active,
     .btn.active,
     .btn.disabled,
     .btn[disabled] {
       background-color: #e6e6e6;
     }

     .btn-large {
       padding: 9px 14px;
       font-size: 15px;
       line-height: normal;
       -webkit-border-radius: 5px;
       -moz-border-radius: 5px;
       border-radius: 5px;
     }

     .btn:hover {
       color: #333333;
       text-decoration: none;
       background-color: #e6e6e6;
       background-position: 0 -15px;
       -webkit-transition: background-position 0.1s linear;
       -moz-transition: background-position 0.1s linear;
       -ms-transition: background-position 0.1s linear;
       -o-transition: background-position 0.1s linear;
       transition: background-position 0.1s linear;
     }

     .btn-primary,
     .btn-primary:hover {
       text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.25);
       color: #ffffff;
     }

     .btn-primary.active {
       color: rgba(255, 255, 255, 0.75);
     }

     .btn-primary {
       background-color: #4a77d4;
       background-image: -moz-linear-gradient(top, #6eb6de, #4a77d4);
       background-image: -ms-linear-gradient(top, #6eb6de, #4a77d4);
       background-image: -webkit-gradient(linear, 0 0, 0 100%, from(#6eb6de), to(#4a77d4));
       background-image: -webkit-linear-gradient(top, #6eb6de, #4a77d4);
       background-image: -o-linear-gradient(top, #6eb6de, #4a77d4);
       background-image: linear-gradient(top, #6eb6de, #4a77d4);
       background-repeat: repeat-x;
       filter: progid:dximagetransform.microsoft.gradient(startColorstr=#6eb6de, endColorstr=#4a77d4, GradientType=0);
       border: 1px solid #3762bc;
       text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.4);
       box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2), 0 1px 2px rgba(0, 0, 0, 0.5);
     }

     .btn-primary:hover,
     .btn-primary:active,
     .btn-primary.active,
     .btn-primary.disabled,
     .btn-primary[disabled] {
       filter: none;
       background-color: #4a77d4;
     }

     .btn-block {
       display: block;
     }

     * {
       -webkit-box-sizing: border-box;
       -moz-box-sizing: border-box;
       -ms-box-sizing: border-box;
       -o-box-sizing: border-box;
       box-sizing: border-box;
     }



     body {

       font-family: 'Open Sans', sans-serif;
       background: white;

     }

     .login {


       left: 40%;
     }

     .login h1 {
       color: #4a77d4;
       text-shadow: 0 0 10px rgba(0, 0, 0, 0.3);
       letter-spacing: 1px;
       text-align: center;
     }

     input {

       margin-bottom: 10px;
       background: white;
       border: none;
       outline: none;
       padding: 10px;
       font-size: 13px;
       color: #4a77d4;
       ;
       text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.3);
       border: 1px solid rgba(0, 0, 0, 0.3);
       border-radius: 4px;
       box-shadow: inset 0 -5px 45px rgba(100, 100, 100, 0.2), 0 1px 1px rgba(255, 255, 255, 0.2);
       -webkit-transition: box-shadow .5s ease;
       -moz-transition: box-shadow .5s ease;
       -o-transition: box-shadow .5s ease;
       -ms-transition: box-shadow .5s ease;
       transition: box-shadow .5s ease;
     }

     input:focus {
       box-shadow: inset 0 -5px 45px rgba(100, 100, 100, 0.4), 0 1px 1px rgba(255, 255, 255, 0.2);
     }
   </style>
   <script language="Javascript">
     function Validate2() {
       var DateCompletionGet = document.getElementById("datec");
       var DateCompletion = new Date(DateCompletionGet.value);
       var currentdate = new Date();
       var currentyear = new Date(currentdate.getFullYear());
       var DateCompletionYear = new Date(DateCompletion.getFullYear());
       var DateCompletionMonth = DateCompletion.getMonth();
       var DateCompletionDate = DateCompletion.getDate();

       var CurrentMonth = currentdate.getMonth();
       var CurrentDate = currentdate.getDate();

       var Completionyeragap = currentyear - DateCompletionYear;
       var Completionmonthgap = CurrentMonth - DateCompletionMonth;
       var CompletionDategap = CurrentDate - DateCompletionDate;

       if ((Completionyeragap < 0)) {
         document.getElementById("info").innerHTML = "Please check paid date";
         DateCompletionGet.focus();
         return false;
       }
       if ((Completionyeragap === 0) && (Completionmonthgap < 0)) {
         document.getElementById("info").innerHTML = "Please check paid date";
         DateCompletionGet.focus();
         return false;
       }
       if ((Completionyeragap === 0) && (Completionmonthgap === 0) && (CompletionDategap < 0)) {
         document.getElementById("info").innerHTML = "Please check paid date";
         DateCompletionGet.focus();
         return false;
       }
       return true;
     }
   </script>
   <style>
     .Class1 {
       font-size: large;
       font-family: sans-serif;
       font-style: normal;

     }

     .Class2 {
       font-size: medium;
       font-family: sans-serif;
       font-style: normal;

     }
   </style>
   <style>
     * {
       box-sizing: border-box;
     }

     .column {
       float: left;
       width: 50%;
       alignment-adjust: central;
       padding: 1px;
       height: 375px;
       /* Should be removed. Only for demonstration */
     }

     .column2 {
       float: left;
       width: 6%;
       padding: 1px;
       height: 250px;
       /* Should be removed. Only for demonstration */
     }

     .row:after {
       content: "";
       display: table;
       clear: both;
       alignment-adjust: text-after-edge;
       height: 100;
       /* Should be removed. Only for demonstration */
     }

     .Break {
       page-break-after: right;
     }

     .backspace {
       margin-left: 25px;
     }

     .TableSize {
       size: A4;
     }

     .SemesterFont {
       font-size: small;
       font-family: serif;
       font-style: normal;
     }

     .HeaderFont {
       font-size: medium;
       font-family: serif;
       font-style: normal;
     }
   </style>
   <style type="text/css">
     button.btn[data-v-f3fb3dc8] {
       display: inline-block;
       font-weight: 300;
       line-height: 1.25;
       text-align: center;
       white-space: nowrap;
       vertical-align: middle;
       user-select: none;
       border: 1px solid transparent;
       cursor: pointer;
       letter-spacing: 1px;
       transition: all 0.15s ease
     }

     button.btn.btn-sm[data-v-f3fb3dc8] {
       padding: 0.4rem 0.8rem;
       font-size: 0.8rem;
       border-radius: 0.2rem
     }

     button.btn.btn-primary[data-v-f3fb3dc8] {
       color: #fff;
       background-color: #45c8f1;
       border-color: #45c8f1
     }

     button.btn.btn-outline-primary[data-v-f3fb3dc8] {
       color: #45c8f1;
       background-color: transparent;
       border-color: #45c8f1
     }

     button.btn.btn-danger[data-v-f3fb3dc8] {
       color: #fff;
       background-color: #ff4949;
       border-color: #ff4949
     }

     .text-muted[data-v-f3fb3dc8] {
       color: #8492a6
     }

     .text-center[data-v-f3fb3dc8] {
       text-align: center
     }

     .drop-down-enter[data-v-f3fb3dc8],
     .drop-down-leave-to[data-v-f3fb3dc8] {
       transform: translateX(0) translateY(-20px);
       transition-timing-function: cubic-bezier(0.74, 0.04, 0.26, 1.05);
       opacity: 0
     }

     .drop-down-enter-active[data-v-f3fb3dc8],
     .drop-down-leave-active[data-v-f3fb3dc8] {
       transition: all 0.15s
     }

     .move-left-enter[data-v-f3fb3dc8],
     .move-left-leave-to[data-v-f3fb3dc8] {
       transform: translateY(0) translateX(-80px);
       transition-timing-function: cubic-bezier(0.74, 0.04, 0.26, 1.05);
       opacity: 0
     }

     .move-left-enter-active[data-v-f3fb3dc8],
     .move-left-leave-active[data-v-f3fb3dc8] {
       transition: all 0.15s
     }

     .no-tr[data-v-f3fb3dc8] {
       transition: none !important
     }

     .no-tr *[data-v-f3fb3dc8] {
       transition: none !important
     }

     .overlay[data-v-f3fb3dc8] {
       position: fixed;
       background: rgba(220, 220, 220, 0.8);
       display: flex;
       align-items: center;
       justify-content: center;
       top: 0;
       left: 0;
       right: 0;
       bottom: 0;
       transition: all 0.2s ease;
       opacity: 0;
       visibility: hidden
     }

     .overlay .modal[data-v-f3fb3dc8] {
       transition: all 0.2s ease;
       opacity: 0;
       transform: scale(0.6);
       overflow: hidden
     }

     .overlay.show[data-v-f3fb3dc8] {
       opacity: 1;
       visibility: visible
     }

     .overlay.show .modal[data-v-f3fb3dc8] {
       opacity: 1;
       transform: scale(1)
     }

     .panel[data-v-f3fb3dc8] {
       padding: 6px 10px;
       display: flex;
       width: 100%;
       box-sizing: border-box;
       align-items: center;
       border-radius: 6px;
       position: relative;
       border: 1px solid #eaf7ff;
       background: #f7fcff;
       outline: none;
       transition: all 0.07s ease-in-out
     }

     .btn[data-v-f3fb3dc8] {
       cursor: pointer;
       box-sizing: border-box
     }

     .light-btn[data-v-f3fb3dc8] {
       padding: 10px 12px;
       display: flex;
       width: 100%;
       box-sizing: border-box;
       align-items: center;
       border-radius: 6px;
       position: relative;
       border: 1px solid #eaf7ff;
       background: #f7fcff;
       outline: none;
       cursor: pointer;
       transition: all 0.07s ease-in-out
     }

     .light-btn[data-v-f3fb3dc8]:hover {
       background: #e0f4ff;
       border-color: #8acfff
     }

     .primary-btn[data-v-f3fb3dc8] {
       background: #239bf5;
       color: white;
       border-radius: 6px;
       height: 46px;
       display: flex;
       align-items: center;
       justify-content: center;
       transition: 0.2s ease;
       font-size: 14px;
       font-family: "Didact Gothic Regular", sans-serif
     }

     .primary-btn[data-v-f3fb3dc8]:hover {
       background: #40a8f6;
       color: white;
       text-decoration: none
     }

     .shake[data-v-f3fb3dc8] {
       animation: shake-f3fb3dc8 0.82s cubic-bezier(0.36, 0.07, 0.19, 0.97) both;
       transform: translate3d(0, 0, 0)
     }

     @keyframes shake-f3fb3dc8 {

       10%,
       90% {
         transform: translate3d(-1px, 0, 0)
       }

       20%,
       80% {
         transform: translate3d(2px, 0, 0)
       }

       30%,
       50%,
       70% {
         transform: translate3d(-4px, 0, 0)
       }

       40%,
       60% {
         transform: translate3d(4px, 0, 0)
       }
     }

     .pulse[data-v-f3fb3dc8] {
       animation: pulse-f3fb3dc8 2s ease infinite
     }

     @keyframes pulse-f3fb3dc8 {
       0% {
         opacity: 0.7
       }

       50% {
         opacity: 0.4
       }

       100% {
         opacity: 0.7
       }
     }

     .flash-once[data-v-f3fb3dc8] {
       animation: flash-once 3.5s ease 1
     }

     @keyframes fade-up-f3fb3dc8 {
       0% {
         transform: translate3d(0, 10px, 0);
         opacity: 0
       }

       100% {
         transform: translate3d(0, 0, 0);
         opacity: 1
       }
     }

     .fade-in[data-v-f3fb3dc8] {
       animation: fade-in-f3fb3dc8 0.3s ease-in-out
     }

     @keyframes fade-in-f3fb3dc8 {
       0% {
         opacity: 0
       }

       100% {
         opacity: 1
       }
     }

     .spin[data-v-f3fb3dc8] {
       animation-name: spin-f3fb3dc8;
       animation-duration: 2000ms;
       animation-iteration-count: infinite;
       animation-timing-function: linear
     }

     @keyframes spin-f3fb3dc8 {
       from {
         transform: rotate(0deg)
       }

       to {
         transform: rotate(360deg)
       }
     }

     .bounceIn[data-v-f3fb3dc8] {
       animation-name: bounceIn-f3fb3dc8;
       transform-origin: center bottom;
       animation-duration: 1s;
       animation-fill-mode: both;
       animation-iteration-count: 1
     }

     @keyframes bounceIn-f3fb3dc8 {

       0%,
       20%,
       40%,
       60%,
       80%,
       100% {
         -webkit-transition-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1);
         transition-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1)
       }

       0% {
         opacity: 1;
         -webkit-transform: scale3d(0.8, 0.8, 0.8);
         transform: scale3d(0.8, 0.8, 0.8)
       }

       20% {
         -webkit-transform: scale3d(1.1, 1.1, 1.1);
         transform: scale3d(1.1, 1.1, 1.1)
       }

       40% {
         -webkit-transform: scale3d(0.9, 0.9, 0.9);
         transform: scale3d(0.9, 0.9, 0.9)
       }

       60% {
         opacity: 1;
         -webkit-transform: scale3d(1.03, 1.03, 1.03);
         transform: scale3d(1.03, 1.03, 1.03)
       }

       80% {
         -webkit-transform: scale3d(0.97, 0.97, 0.97);
         transform: scale3d(0.97, 0.97, 0.97)
       }

       100% {
         opacity: 1;
         -webkit-transform: scale3d(1, 1, 1);
         transform: scale3d(1, 1, 1)
       }
     }

     @keyframes dots-f3fb3dc8 {

       0%,
       20% {
         color: rgba(0, 0, 0, 0);
         text-shadow: 0.25em 0 0 rgba(0, 0, 0, 0), 0.5em 0 0 rgba(0, 0, 0, 0)
       }

       40% {
         color: #8492a6;
         text-shadow: 0.25em 0 0 rgba(0, 0, 0, 0), 0.5em 0 0 rgba(0, 0, 0, 0)
       }

       60% {
         text-shadow: 0.25em 0 0 #8492a6, 0.5em 0 0 rgba(0, 0, 0, 0)
       }

       80%,
       100% {
         text-shadow: 0.25em 0 0 #8492a6, 0.5em 0 0 #8492a6
       }
     }

     @keyframes recording-f3fb3dc8 {
       0% {
         box-shadow: 0px 0px 5px 0px rgba(173, 0, 0, 0.3)
       }

       65% {
         box-shadow: 0px 0px 5px 5px rgba(173, 0, 0, 0.3)
       }

       90% {
         box-shadow: 0px 0px 5px 5px rgba(173, 0, 0, 0)
       }
     }

     @font-face {
       font-family: 'Didact Gothic Regular';
       src: url(data:font/woff2;base64,) format("woff");
       font-weight: normal;
       font-style: normal
     }

     body[data-v-f3fb3dc8] {
       margin: 0;
       font-size: 100%;
       color: #3c4858
     }

     a[data-v-f3fb3dc8] {
       text-decoration: none;
       color: #45c8f1
     }

     h1[data-v-f3fb3dc8],
     h2[data-v-f3fb3dc8],
     h3[data-v-f3fb3dc8],
     h4[data-v-f3fb3dc8] {
       margin-top: 0
     }

     svg[data-v-f3fb3dc8] {
       outline: none
     }

     .container_selected_area[data-v-f3fb3dc8] {
       display: none;
       visibility: hidden;
       padding: 0;
       position: fixed;
       top: 0;
       left: 0;
       right: 0;
       bottom: 0;
       z-index: 2147483647
     }

     .container_selected_area.active[data-v-f3fb3dc8] {
       visibility: visible;
       display: block
     }

     .container_selected_area .label[data-v-f3fb3dc8] {
       font-family: "Didact Gothic Regular", sans-serif;
       font-size: 22px;
       text-align: center;
       padding-top: 15px
     }

     .area[data-v-f3fb3dc8] {
       display: none;
       position: absolute;
       z-index: 2147483647;
       pointer-events: none;
       border: 1px solid #1e83ff;
       background: rgba(30, 131, 255, 0.1);
       box-sizing: border-box
     }

     .area.active[data-v-f3fb3dc8] {
       display: block;
       top: 0;
       left: 0
     }

     .hide[data-v-f3fb3dc8] {
       display: none
     }
   </style>
   <style type="text/css">
     @font-face {
       font-weight: 400;
       font-style: normal;
       font-family: 'Circular-Loom';

       src: url('chrome-extension://liecbddmkiiihnedobmlmillhodjkdmb/fonts/CircularXXWeb-Book.woff2') format('woff2');
     }

     @font-face {
       font-weight: 500;
       font-style: normal;
       font-family: 'Circular-Loom';

       src: url('chrome-extension://liecbddmkiiihnedobmlmillhodjkdmb/fonts/CircularXXWeb-Medium.woff2') format('woff2');
     }

     @font-face {
       font-weight: 700;
       font-style: normal;
       font-family: 'Circular-Loom';

       src: url('chrome-extension://liecbddmkiiihnedobmlmillhodjkdmb/fonts/CircularXXWeb-Bold.woff2') format('woff2');
     }

     @font-face {
       font-weight: 900;
       font-style: normal;
       font-family: 'Circular-Loom';

       src: url('chrome-extension://liecbddmkiiihnedobmlmillhodjkdmb/fonts/CircularXXWeb-Black.woff2') format('woff2');
     }
   </style>
 </head>

 <body data-new-gr-c-s-check-loaded="14.1087.0" data-gr-ext-installed="">
   <table align="center" width="100%" height="25px" class="TableSize">
     <tbody>
       <tr>
         <td align="center" colspan="3">
           <b class="Class1">Addis Ababa Medical and Business College</b><br>
           <b class="Class2">Office of the Registrar</b><br>
           <b class="Class1">Students' academic record</b><br>
           <b>Tel:</b> +251-111-56-0005, <b>Email:</b> registraroffice@aambc.net<br> <b>P.O.Box:</b> 56057
           <b>Addis Ababa, Ethiopia</b>
         </td>
       </tr>
       <tr>
         <td colspan="3">
           <table width="100%" border="1px">
             <tbody>
               <tr>
                 <td><b>Full name: </b><?php echo $student_info_row[0]['full_name']; ?></td>
                 <td class="HeaderFont"><b>Sex: </b><?php echo $student_info_row[0]['gender']; ?></td>
                 <td class="HeaderFont"><b>ID. Number:
                   </b><?php echo $student_info_row[0]['id_number']; ?></td>
                 <td class="HeaderFont"><b>Birth date:
                   </b><?php echo $student_info_row[0]['birth_date']; ?></td>
                 <td class="HeaderFont"><b>Medium of instruction: </b>English</td>
               </tr>
               <tr>
                 <td class="HeaderFont"><b>Major: </b>Midwifery</td>
                 <td class="HeaderFont"> <b>Minor: </b>-</td>
                 <td class="HeaderFont"><b>Program level: </b>BSc. degree</td>
                 <td class="HeaderFont"><b>Admission classification: </b>Extension</td>
                 <td class="HeaderFont"><b>Graduated date: </b>October 21, 2022</td>
               </tr>

               <tr>
                 <td colspan="6" class="SemesterFont">
                   <div class="row">
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <span class="tans-nurs"> <b><u>Transfer From BSC Nursing</u> </b> </span>

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: I of 2010E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               ANAT 201
                             </td>
                             <td>
                               Human Anatomy
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo  strtoupper($grade_row[0]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[0]['grade'], $grade_row[0]['course_id']); ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               SOSA201
                             </td>
                             <td>
                               Introduction to Sociology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[1]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[1]['grade'], $grade_row[1]['course_id']); ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               FLEn 101
                             </td>
                             <td>
                               Communicating English
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[2]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[2]['grade'], $grade_row[2]['course_id']); ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               PHYL 201
                             </td>
                             <td>
                               Human Physiology
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[3]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[3]['grade'], $grade_row[3]['course_id']); ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Epsy 201
                             </td>
                             <td>
                               General Psychology
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[4]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[4]['grade'], $grade_row[4]['course_id']); ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Chem 202
                             </td>
                             <td>
                               Biochemistry
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[5]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester11[] = get_grade_point($grade_row[5]['grade'], $grade_row[5]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>19</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester11); ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester11) / 19, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint = 0;
                                    $newpoint += array_sum($semester11);
                                    echo $for_status = round($newpoint / 19, 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status:
                               </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <span class="tans-nurs"> <b><u>Transfer From BSC Nursing</u></b> </span>

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: II of 2010E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Micro 222
                             </td>
                             <td>
                               Microbiology &amp; immunology
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[6]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[6]['grade'], $grade_row[6]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 282
                             </td>
                             <td>
                               Pathophysiology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[7]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[7]['grade'], $grade_row[7]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               PARA 321
                             </td>
                             <td>
                               Parasitology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[8]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[8]['grade'], $grade_row[8]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 231
                             </td>
                             <td>
                               Professional Ethics
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[9]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[9]['grade'], $grade_row[9]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 202
                             </td>
                             <td>
                               Fundamentals of Nursing
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[10]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[10]['grade'], $grade_row[10]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS211
                             </td>
                             <td>
                               First aid and Accident Prevention
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[11]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester12[] = get_grade_point($grade_row[11]['grade'], $grade_row[11]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>17</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester12) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester12) / 17, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester12);
                                    echo $for_status = round($newpoint / (19 + 17), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <span class="tans-nurs"> <b><u>Transfer From BSC Nursing</u> </b> </span>

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: I of 2011E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>


                           <tr>
                             <td align="left">
                               GeEd
                             </td>
                             <td>
                               Civics and ethical education
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[12]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester21[] = get_grade_point($grade_row[12]['grade'], $grade_row[12]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 331
                             </td>
                             <td>
                               Operation Room Technique
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[13]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester21[] = get_grade_point($grade_row[13]['grade'], $grade_row[13]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Pharm 302
                             </td>
                             <td>
                               Pharmacology for midwives
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[14]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester21[] = get_grade_point($grade_row[14]['grade'], $grade_row[14]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 252
                             </td>
                             <td>
                               Nutrition in Health and Illness
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[15]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester21[] = get_grade_point($grade_row[15]['grade'], $grade_row[15]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>12</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester21) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester21) / 12, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester21);
                                    echo $for_status = round($newpoint / (19 + 17 + 12), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <span class="tans-nurs"> <b><u>Admitted To Midwifery</u></b></span>
                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: II of 2011E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS301
                             </td>
                             <td>
                               Medical Surgical Nursing
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[16]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[16]['grade'], $grade_row[16]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS341
                             </td>
                             <td>
                               Health Assessment in midwifery
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[17]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[17]['grade'], $grade_row[17]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 321
                             </td>
                             <td>
                               Obstetrics I
                             </td>
                             <td align="center">
                               <b>4</b>
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[18]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[18]['grade'], $grade_row[18]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 312
                             </td>
                             <td>
                               Obstetrics Practicum I
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[19]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[19]['grade'], $grade_row[19]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Nurs311
                             </td>
                             <td>
                               Medical surgical Nursing Practicum
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[20]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[20]['grade'], $grade_row[20]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 322
                             </td>
                             <td>
                               Neonatology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[21]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[21]['grade'], $grade_row[21]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 471
                             </td>
                             <td>
                               Neonatology practice
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[22]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester22[] = get_grade_point($grade_row[22]['grade'], $grade_row[22]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>21</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester22) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester22) / 21, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester22);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                   </div>
                 </td>
               </tr>
             </tbody>
           </table>
         </td>
       </tr>
       <tr>
         <td colspan="3" class="SemesterFont"><b>Grading system: </b>A+ =Excellent , A =Excellent , A- =Very
           good , B+ =Very good , B =Good , B- =Good , C+ =Satisfactory , C =Satisfactory , C- =Satisfactory ,
           D =Unsatisfactory , F =Failed , I =Incomplete , NG =No grade , Ex. =Exempted , <sup>*</sup>
           =Repeated; <b class="backspace">



             <b>Points: </b>A+ =4.0, A =4.0, A- =3.75, B+ =3.5, B =3.0, B- =2.75, C+ =2.5, C =2.0, C- =1.75,
             D =1.0, F =0, I =0, NG =0</b></td>
       </tr>
       <tr>
         <td><b>Note: </b> This transcript is official only when signed and sealed by the Registrar</td>
         <td align="center"><b>Registrar</b><br>
           _____________________
         </td>
         <td align="center"><b>Date of issued</b><br>
           _____________________
         </td>
       </tr>
       <tr>
         <td align="center">

         </td>
       </tr>
     </tbody>
   </table>

   <p style="page-break-before: always">
   </p>
   <table align="center" width="100%" class="TableSize">
     <tbody>
       <tr>
         <td align="center" colspan="3">
           <b class="Class1">Addis Ababa Medical and Business College</b><br>
           <b class="Class2">Office of the Registrar</b><br>
           <b class="Class1">Students' academic record</b><br>
           <b>Tel:</b> +251-111-56-0005, <b>Email:</b> registraroffice@aambc.net<br> <b>P.O.Box:</b> 56057
           <b>Addis Ababa, Ethiopia</b>
         </td>
       </tr>
       <tr>
         <td colspan="3">
           <table width="100%" border="1px">
             <tbody>
               <tr>
                 <td><b>Full name: </b><?php echo $student_info_row[0]['full_name']; ?></td>
                 <td class="HeaderFont"><b>Sex: </b><?php echo $student_info_row[0]['gender']; ?></td>
                 <td class="HeaderFont"><b>ID. Number:
                   </b><?php echo $student_info_row[0]['id_number']; ?></td>
                 <td class="HeaderFont"><b>Birth date:
                   </b><?php echo $student_info_row[0]['birth_date']; ?></td>
                 <td class="HeaderFont"><b>Medium of instruction: </b>English</td>
               </tr>
               <tr>
                 <td class="HeaderFont"><b>Major: </b>Midwifery</td>
                 <td class="HeaderFont"> <b>Minor: </b>-</td>
                 <td class="HeaderFont"><b>Program level: </b>BSc. degree</td>
                 <td class="HeaderFont"><b>Admission classification: </b>Extension</td>
                 <td class="HeaderFont"><b>Graduated date: </b>October 21, 2022</td>
               </tr>

               <tr>
                 <td colspan="6" class="SemesterFont">
                   <div class="row">
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: I of 2012E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               MIDW 571
                             </td>
                             <td>
                               Psychiatric in midwifery
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[23]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[23]['grade'], $grade_row[23]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 323
                             </td>
                             <td>
                               Obstetrics II
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[25]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[25]['grade'], $grade_row[25]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 451
                             </td>
                             <td>
                               Pediatrics
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[24]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[24]['grade'], $grade_row[24]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 441
                             </td>
                             <td>
                               Health Education
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[26]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[26]['grade'], $grade_row[26]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 412
                             </td>
                             <td>
                               Gynecology
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[27]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[27]['grade'], $grade_row[27]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Comp2022
                             </td>
                             <td>
                               Introduction to Computer
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[28]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester31[] = get_grade_point($grade_row[28]['grade'], $grade_row[28]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>18</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester31) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester31) / 18, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester31);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: II of 2012E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               MIDW 372
                             </td>
                             <td>
                               Obstetric ultrasound
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[29]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[29]['grade'], $grade_row[29]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               FLEN201
                             </td>
                             <td>
                               Sophomore English/Writing Skills
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[30]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[30]['grade'], $grade_row[30]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               nurs442
                             </td>
                             <td>
                               Communicable Disease Control
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[31]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[31]['grade'], $grade_row[31]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 481
                             </td>
                             <td>
                               Environmental health
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[32]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[32]['grade'], $grade_row[32]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 472
                             </td>
                             <td>
                               Reproductive Health
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[33]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[33]['grade'], $grade_row[33]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 521
                             </td>
                             <td>
                               Epidemiology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[34]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester32[] = get_grade_point($grade_row[34]['grade'], $grade_row[34]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>16</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester32) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester32) / 16, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester32);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18 + 16), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: I of 2013E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               MIDW 552
                             </td>
                             <td>
                               Teaching methodology and curriculum development
                             </td>
                             <td align="center">
                               4
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[35]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[35]['grade'], $grade_row[35]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 431
                             </td>
                             <td>
                               Obstetrics II practicum
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[36]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[36]['grade'], $grade_row[36]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW311
                             </td>
                             <td>
                               Family Planning
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[37]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[37]['grade'], $grade_row[37]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Stat 322
                             </td>
                             <td>
                               Biostatics
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[38]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[38]['grade'], $grade_row[38]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 482
                             </td>
                             <td>
                               Obstetrics III
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[39]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[39]['grade'], $grade_row[39]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 462
                             </td>
                             <td>
                               Research Methodology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[40]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester41[] = get_grade_point($grade_row[40]['grade'], $grade_row[40]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>17</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester41) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester41) / 17, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester41);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18 + 16 + 17), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">

                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: II of 2013E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               MIDW525
                             </td>
                             <td>
                               Health Service Management
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[41]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester42[] = get_grade_point($grade_row[41]['grade'], $grade_row[41]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW552
                             </td>
                             <td>
                               Gynecological clinic(Internship)
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[42]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester42[] = get_grade_point($grade_row[42]['grade'], $grade_row[42]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 522
                             </td>
                             <td>
                               Delivery &amp; postnatal Ward
                             </td>
                             <td align="center">
                               3
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[43]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester42[] = get_grade_point($grade_row[43]['grade'], $grade_row[43]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               PHYL 202
                             </td>
                             <td>
                               Reproductive Physiology
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[44]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester42[] = get_grade_point($grade_row[44]['grade'], $grade_row[44]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               ANAT 202
                             </td>
                             <td>
                               Reproductive Anatomy
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[45]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester42[] = get_grade_point($grade_row[45]['grade'], $grade_row[45]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>12</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester42) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester42) / 12, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester42);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18 + 16 + 17 + 12), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                   </div>
                 </td>
               </tr>
             </tbody>
           </table>
         </td>
       </tr>
       <tr>
         <td colspan="3" class="SemesterFont"><b>Grading system: </b>A+ =Excellent , A =Excellent , A- =Very
           good , B+ =Very good , B =Good , B- =Good , C+ =Satisfactory , C =Satisfactory , C- =Satisfactory ,
           D =Unsatisfactory , F =Failed , I =Incomplete , NG =No grade , Ex. =Exempted , <sup>*</sup>
           =Repeated; <b class="backspace">



             <b>Points: </b>A+ =4.0, A =4.0, A- =3.75, B+ =3.5, B =3.0, B- =2.75, C+ =2.5, C =2.0, C- =1.75,
             D =1.0, F =0, I =0, NG =0</b></td>
       </tr>
       <tr>
         <td><b>Note: </b> This transcript is official only when signed and sealed by the Registrar</td>
         <td align="center"><b>Registrar</b><br>
           _____________________
         </td>
         <td align="center"><b>Date of issued</b><br>
           _____________________
         </td>
       </tr>
       <tr>
         <td align="center">

         </td>
       </tr>
     </tbody>
   </table>

   <p style="page-break-before: always">
   </p>
   <table align="center" width="100%" class="TableSize">
     <tbody>
       <tr>
         <td align="center" colspan="3">
           <b class="Class1">Addis Ababa Medical and Business College</b><br>
           <b class="Class2">Office of the Registrar</b><br>
           <b class="Class1">Students' academic record</b><br>
           <b>Tel:</b> +251-111-56-0005, <b>Email:</b> registraroffice@aambc.net<br> <b>P.O.Box:</b> 56057
           <b>Addis Ababa, Ethiopia</b>
         </td>
       </tr>
       <tr>
         <td colspan="3">
           <table width="100%" border="1px">
             <tbody>
               <tr>
                 <td><b>Full name: </b><?php echo $student_info_row[0]['full_name']; ?></td>
                 <td class="HeaderFont"><b>Sex: </b>F</td>
                 <td class="HeaderFont"><b>ID. Number:
                   </b><?php echo $student_info_row[0]['id_number']; ?></td>
                 <td class="HeaderFont"><b>Birth date:
                   </b><?php echo $student_info_row[0]['birth_date']; ?></td>
                 <td class="HeaderFont"><b>Medium of instruction: </b>English</td>
               </tr>
               <tr>
                 <td class="HeaderFont"><b>Major: </b>Midwifery</td>
                 <td class="HeaderFont"> <b>Minor: </b>-</td>
                 <td class="HeaderFont"><b>Program level: </b>BSc. degree</td>
                 <td class="HeaderFont"><b>Admission classification: </b>Extension</td>
                 <td class="HeaderFont"><b>Graduated date: </b>October 21, 2022</td>
               </tr>

               <tr>
                 <td colspan="6" class="SemesterFont">
                   <div class="row">
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: I of 2014E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>



                           <tr>
                             <td align="left">
                               MIDW 542
                             </td>
                             <td>
                               Community Midwifery practice
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[46]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[46]['grade'], $grade_row[46]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 382
                             </td>
                             <td>
                               Obstetrics III Practicum
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[47]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[47]['grade'], $grade_row[47]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               Nurs3144
                             </td>
                             <td>
                               Pediatric practicum
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[48]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[48]['grade'], $grade_row[48]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 561
                             </td>
                             <td>
                               Student Research project proposal I
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[49]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[49]['grade'], $grade_row[49]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 573
                             </td>
                             <td>
                               Psychiatric in midwifery Practicum
                             </td>
                             <td align="center">
                               1
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[50]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[50]['grade'], $grade_row[50]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 532
                             </td>
                             <td>
                               Ante-natal clinic
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[51]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester51[] = get_grade_point($grade_row[51]['grade'], $grade_row[51]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>11</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester51) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester51) / 11, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester51);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18 + 16 + 17 + 12 + 11), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>



                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                     <div class="column" align="center" style="background-color:white;">
                       <table align="center" class="SemesterFont">
                         <tbody>
                           <tr>
                             <td colspan="5" align="left">
                               <b><u>Semester: II of 2014E.C. academic year</u></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="left">
                               <b><u>Course code</u></b>
                             </td>
                             <td align="center">
                               <b><u>Course title</u></b>
                             </td>
                             <td align="center">
                               <b><u>Credit hour</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade</u></b>
                             </td>
                             <td align="center">
                               <b><u>Grade point</u></b>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               NURS 522
                             </td>
                             <td>
                               Student research project II
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[52]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester52[] = get_grade_point($grade_row[52]['grade'], $grade_row[52]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td align="left">
                               MIDW 562
                             </td>
                             <td>
                               Neonatology + pediatrics practicum Comprehensive Exam
                             </td>
                             <td align="center">
                               2
                             </td>
                             <td align="center">
                               <?php echo strtoupper($grade_row[53]['grade']); ?>
                             </td>
                             <td align="center">
                               <?php echo $semester52[] = get_grade_point($grade_row[53]['grade'], $grade_row[53]['course_id']);
                                ?>
                             </td>
                           </tr>

                           <tr>
                             <td colspan="2" align="right"><b>Semester total</b></td>
                             <td align="center"><b>4</b></td>
                             <td>

                             </td>
                             <td align="center">
                               <b><?php echo array_sum($semester52) ?></b>
                             </td>
                           </tr>
                           <tr>
                             <td align="center" colspan="2">
                               <p></p><b>Semester GPA:
                               </b><?php echo round(array_sum($semester52) / 4, 2); ?><br>
                               <b>Cumulative GPA:
                               </b><?php $newpoint += array_sum($semester52);
                                    echo $for_status = round($newpoint / (19 + 17 + 12 + 21 + 18 + 16 + 17 + 12 + 11 + 4), 2); ?>
                             </td>
                             <td align="center" colspan="3"><b>Status: </b><?php echo get_status($for_status);  ?><br>
                               <?php
                                /// end of While to fetch data
                                //}
                                ?>
                               <b style="background-color: white;color: black"></b><br>
                               <b style="background-color: white;color: black"></b>
                             </td>
                           </tr>
                         </tbody>
                       </table>
                     </div>
                   </div>
                 </td>
               </tr>
             </tbody>
           </table>
         </td>
       </tr>
       <tr>
         <td colspan="3" class="SemesterFont"><b>Grading system: </b>A+ =Excellent , A =Excellent , A- =Very
           good , B+ =Very good , B =Good , B- =Good , C+ =Satisfactory , C =Satisfactory , C- =Satisfactory ,
           D =Unsatisfactory , F =Failed , I =Incomplete , NG =No grade , Ex. =Exempted , <sup>*</sup>
           =Repeated; <b class="backspace">



             <b>Points: </b>A+ =4.0, A =4.0, A- =3.75, B+ =3.5, B =3.0, B- =2.75, C+ =2.5, C =2.0, C- =1.75,
             D =1.0, F =0, I =0, NG =0</b></td>
       </tr>
       <tr>
         <td><b>Note: </b> This transcript is official only when signed and sealed by the Registrar</td>
         <td align="center"><b>Registrar</b><br>
           _____________________
         </td>
         <td align="center"><b>Date of issued</b><br>
           _____________________
         </td>
       </tr>
       <tr>
         <td align="center">

         </td>
       </tr>
     </tbody>
   </table>



   <iframe src="./Students transcript_SMIS-Addis Ababa Medical and Business College_files/audio-devices.html" allow="microphone" style="display: none;"></iframe><input type="file" id="" name="file" style="display: none;">
   <div data-v-f3fb3dc8="">
     <div data-v-f3fb3dc8="" class="container_selected_area" style="cursor: crosshair;"></div>
     <div data-v-f3fb3dc8="" class="area" style="left: 0px; top: 0px; width: 0px; height: 0px;"></div>
   </div>
   <div id="scrnli_recorder_root"></div>
 </body>
 <loom-container id="lo-engage-ext-container">
   <loom-shadow classname="resolved"></loom-shadow>
 </loom-container>
 <loom-container id="lo-companion-container">
   <loom-shadow classname="resolved"></loom-shadow>
 </loom-container>
 <grammarly-desktop-integration data-grammarly-shadow-root="true"></grammarly-desktop-integration>

 <script src="disable_devtool.js"></script>

 </html>